...
A diferencia de otros lenguajes de programación, los números en Python 3 soportan \link{https://stackoverflow.com/questions/13795758/what-is-sys-maxint-in-python-3}{cualquier cantidad de dígitos}, o sea, son de largo arbitrario. Esto hace que los cálculos con números grandes sean mucho más convenientes, pero reduce el rendimiento.
...
Las cadenas de caracteres se crean introduciendo el texto entre comillas simples \ttt{' '} o dobles \ttt{" "}.
...
A veces es necesario inicializa un string, pero sin agregarle información. Una cadena vacía es definida como \ttt{''} o \ttt{""}.
...
Si el string se define entre comillas dobles, no es necesario poner \ttt{'} para ingresar comillas simples dentro de él, y viceversa.
...
Este es un tipo especial de string, que se escribe entre comillas triples \ttt{''' '''} o \ttt{""" """}, y que reconoce los saltos de línea sin necesidad de usar la secuencia \textbackslash n.
...
\begin{itemize}
  \item Igual $=$, escrito como \ttt{==}: Se evalúa como True si ambos elementos son iguales.
  
  \item Distinto $\neq$, escrito como \ttt{!=}: Se evalúa como True si ambos elementos son distintos.
  
  \item Mayor que $>$, escrito como \ttt{>}: Se evalúa como True si el primer elemento es mayor.
  
  \item Menor que $<$, escrito como \ttt{<}: Se evalúa como True si el primer elemento es menor.
  
  \item Mayor o igual que $\geq$, escrito como \ttt{>=}: Se evalúa como True si el primer elemento es mayor o igual al segundo.
  
  \item Menor o igual que $\leq$, escrito como \ttt{<=}: Se evalúa como True si el primer elemento es menor o igual al segundo.
...
Al igual que otros valores \doble{vacíos}, tales como \ttt{()}, \ttt{[]}, \ttt{\{\}} y la cadena vacía \ttt{""} o \ttt{''}, es False cuando es convertido a una variable booleana.
...
Esto permite saber con facilidad qué métodos se pueden usar con la varible que se está usando.
...
Por defecto, el método \ttt{help()} sólo mostrará el texto que alcance en el alto de la ventana del terminal. Al pulsar la tecla \doble{Intro}, se mostrará la línea que viene después de \ttt{-- Más  --}. Como siempre, se puede pulsar Ctrl+C para salir.\smallskip
