...
\pythonfile{codigo/itertools/itertools.py}
...
\pythonfile{codigo/itertools/count_1.py}
\out{codigo/itertools/count_1.out}
...
\pythonfile{codigo/itertools/count_2.py}
\out{codigo/itertools/count_2.out}

Si el segundo parámetro es negativo, cuenta hacia atrás.

\pythonfile{codigo/itertools/count_3.py}
\out{codigo/itertools/count_3.out}

La función\ttt{cycle()} itera infinitamente a través de un iterable (como una lista o cadena).

\pythonfile{codigo/itertools/cycle_1.py}
\out{codigo/itertools/cycle_1.out}
\pythonfile{codigo/itertools/cycle_2.py}
\out{codigo/itertools/cycle_2.out}

La función \ttt{repeat()} repite un objeto, ya sea infinitamente o un número específico de veces.

\pythonfile{codigo/itertools/repeat_1.py}
\out{codigo/itertools/repeat_1.out}

Puede usarse para crear listas que tengan un objeto repetido.

\pythonfile{codigo/itertools/repeat_2.py}
\out{codigo/itertools/repeat_2.out}

Para repetir infinitamente, se le debe entregar sólo el objeto como parámetro.

\pythonfile{codigo/itertools/repeat_3.py}
\out{codigo/itertools/repeat_3.out}
...
\pythonfile{codigo/itertools/accumulate.py}
\out{codigo/itertools/accumulate.out}

La función \ttt{takewhile()} toma elementos de un iterable mientras una función predicado permanece verdadera.

\pythonfile{codigo/itertools/takewhile_1.py}
\out{codigo/itertools/takewhile_1.out}
...
\pythonfile{codigo/itertools/takewhile_2.py}
\out{codigo/itertools/takewhile_2.out}

La función \ttt{chain()} combina varios iterables en uno solo más largo.

\pythonfile{codigo/itertools/chain.py}
\out{codigo/itertools/chain.out}
...
\pythonfile{codigo/itertools/product.py}
\out{codigo/itertools/product.out}

La función \ttt{permutations()} retorna las todas permutaciones posibles entre los elementos de un iterable.

\pythonfile{codigo/itertools/permutations_1.py}
\out{codigo/itertools/permutations_1.out}
\pythonfile{codigo/itertools/permutations_2.py}
\out{codigo/itertools/permutations_2.out}

Se le puede entregar un segundo argumento, el cual representa el número de elementos que cada permutación debe tener.

\pythonfile{codigo/itertools/permutations_3.py}
\out{codigo/itertools/permutations_3.out}
...
\pythonfile{codigo/itertools/combinations.py}
\out{codigo/itertools/combinations.out}
