...
\pythonfile{codigo/funciones/funciones.py}
...
Para definir una función, se debe usar la palabra clave \ttt{def}, seguida del nombre de la función y de un paréntesis que puede o no incluir parámetros.
...
\pythonfile{codigo/funciones/saludar.py}
...
\pythonfile{codigo/funciones/exclamar.py}

Una función puede pedir más de un argumento, los cuales deben separarse por comas.

\pythonfile{codigo/funciones/sumar.py}
\pythonfile{codigo/funciones/mayor.py}
...
\pythonfile{codigo/funciones/llamar_saludar.py}
\out{codigo/funciones/llamar_saludar.out}
\pythonfile{codigo/funciones/llamar_exclamar.py}
\out{codigo/funciones/llamar_exclamar.out}
\pythonfile{codigo/funciones/llamar_sumar.py}
\out{codigo/funciones/llamar_sumar.out}
\pythonfile{codigo/funciones/llamar_mayor.py}
\out{codigo/funciones/llamar_mayor.out}
...
\pythonfile{codigo/funciones/argumentos.py}
\out{codigo/funciones/argumentos.out}
...
\pythonfile{codigo/funciones/local_y_global.py}
...
Para hacer esto, se debe usar la palabra clave \ttt{global}.

\pythonfile{codigo/funciones/global.py}
\out{codigo/funciones/global.out}

Es importante darse cuenta de que la función \ttt{funcion()} debe ser llamada para inicializar la variable global \ttt{z} y que al usar la palabra clave \ttt{global} se debe separar la declaración de la asignación.

Si no se separan, se mostrará un \ttt{SyntaxError}.

\pythonfile{codigo/funciones/error_global.py}
\out{codigo/funciones/error_global.out}

\Needspace{8\baselineskip}
\section{Devolución de valores en una función}

Ciertas funciones devuelven un valor para ser utilizado más adelante.
Para hacer esto en la definición de funciones nuevas, se debe usar la palabra clave \ttt{return}.

\pythonfile{codigo/funciones/return_1.py}
\out{codigo/funciones/return_1.out}

Cualquier código luego de la sentencia \ttt{return} nunca ocurrirá.

\pythonfile{codigo/funciones/return_2.py}
\out{codigo/funciones/return_2.out}
...
\pythonfile{codigo/funciones/docstring_1.py}

Normalmente se usan docstrings para explicar los parámetros y lo que retorna una función.

%TODO: buscar formato aceptado
\pythonfile{codigo/funciones/docstring_2.py}

El intérprete de Python ignora los docstring que se usen en otros lugares.
Esto ha hecho que algunas personas los usen como \doble{comentarios multilínea}, pero en la práctica no se recomienda hacer esto porque puede generar confusión.

\pythonfile{codigo/funciones/comentario_malo.py}
...
\pythonfile{codigo/funciones/print_doc.py}
\out{codigo/funciones/print_doc.out}

Otra forma es usando la función \ttt{help()}.
Esto entrega un poco más de información.

\pythonfile{codigo/funciones/help.py}
\out{codigo/funciones/help.out}
...
\pythonfile{codigo/funciones/funcion_como_objeto_1.py}
\out{codigo/funciones/funcion_como_objeto_1.out}

Las funciones también pueden ser usadas como argumentos de otras funciones.

\pythonfile{codigo/funciones/funcion_como_objeto_2.py}
\out{codigo/funciones/funcion_como_objeto_2.out}

\Needspace{8\baselineskip}
\section{El objeto None}

El objeto \ttt{None} es utilizado para representar la ausencia de un valor.
Es similar a \ttt{null} en otros lenguajes de programación.
...
Usando \ttt{print()} se mostrará su verdadero valor.

\pythonfile{codigo/funciones/None_1.py}
\out{codigo/funciones/None_1.out}

El objeto \ttt{None} es devuelto por cualquier función que no devuelve explícitamente algo más.

\pythonfile{codigo/funciones/None_2.py}
\out{codigo/funciones/None_2.out}

Un uso común de \ttt{None} es para iniciar una lista con cierta cantidad de elementos.

\pythonfile{codigo/funciones/None_3.py}
\out{codigo/funciones/None_3.out}
...
Para sobrecargar una función, se deben definir los parámetros que sean opcionales con un valor por defecto \ttt{None}.
Después, se usan declaraciones \ttt{if-elif-else} para revisar todos los casos posibles.

\pythonfile{codigo/funciones/sobrecarga_1.py}
\out{codigo/funciones/sobrecarga_1.out}
...
\pythonfile{codigo/funciones/sobrecarga_2.py}
\out{codigo/funciones/sobrecarga_2.out}
...
\pythonfile{codigo/funciones/anotaciones_1.py}
\out{codigo/funciones/anotaciones_1.out}

En el ejemplo de arriba, se muestra que mensaje es un parámetro de tipo string y que la función \ttt{duplicar()} retorna un string.
...
\pythonfile{codigo/funciones/anotaciones_2.py}
\out{codigo/funciones/anotaciones_2.out}
