...
El argumento de la función \ttt{open()} es la ruta del archivo. Esta ruta puede ser \link{https://www.discoduroderoer.es/rutas-relativas-y-absolutas/}{relativa} (desde el directorio de trabajo) o \link{https://www.discoduroderoer.es/rutas-relativas-y-absolutas/}{absoluta}.\smallskip
...
\begin{itemize}
  ...
  \item \ttt{\qq w \qq}: Significa modo de escritura, el cual sirve para reescribir los contenidos de un archivo. Abrir un archivo en este modo inmediatamente borra todos sus contenidos.
...
\begin{itemize}
  \item \ttt{\qq t \qq}: significa abrir el archivo en modo texto (predeterminado). No es necesario escribirlo.
  
  \item \ttt{\qq b \qq}: significa abrir el archivo en modo binario, que es utilizado para archivos que no son de texto (tales como archivos de imágenes o sonido). Se debe combinar con alguno de los modos anteriores.
...
Se puede añadir el signo \ttt{\qq + \qq} a cualquiera de los modos de arriba para darles acceso adicional a archivos.\smallskip

La siguiente tabla muestra el funcionamiento de cada modo:\medskip\medskip
...
El ejemplo de arriba primero mostrará los bytes del \ttt{1} al \ttt{16}, después del \ttt{17} al \ttt{24}, del \ttt{25} al \ttt{28}, y del \ttt{29} al último byte.\smallskip
...
Sin embargo, existe una forma más cómoda de hacer esto usando declaraciones \ttt{with}.\smallskip
...
El archivo se cierra automáticamente al final de la declaración \ttt{with}, incluso si ocurren excepciones dentro de ella.\smallskip
...
Comparado con la forma tradicional de trabajar con archivos (usar \ttt{open()} y después \ttt{close()}), usar \ttt{with} tiene el inconveniente de que los archivos tienen que volverse a abrir cada vez que se quiera trabajar con ellos.\smallskip
